...
export function KycDocumentsTab({ merchant }: { merchant: Merchant }) {
 ...
 {documents.map((doc) => (
 <div key={doc.id} className="p-4 sm:p-5 hover:bg-card/30 transition-colors dark:hover:bg-card/10">
 ...
 <p className="mt-0.5 text-[12px] text-muted-foreground">{doc.type}</p>
 </div>
 </div>
 ...
 <div className="flex justify-between sm:block">
 <p className="text-[12px] text-muted-foreground uppercase tracking-wider">Verified Name</p>
 <p className="sm:mt-1 text-[13px] font-medium text-foreground dark:text-white">{doc.verifiedName}</p>
 </div>
