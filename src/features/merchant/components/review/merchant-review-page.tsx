...
export function MerchantReviewPage({ merchantId }: { merchantId: string }) {
 ...
 if (isLoading) {
 return <div className="p-8 text-center text-muted-foreground">Loading merchant details for review...</div>;
 }
 ...
 <div>
 <h2 className="text-lg font-semibold text-foreground dark:text-white">{merchant.businessName}</h2>
 <div className="flex items-center gap-3 mt-1 flex-wrap">
 <span className="text-sm text-muted-foreground font-medium">ID: {merchant.merchantCode}</span>
 <span className="text-sm text-muted-foreground">•</span>
 <span className="text-[13px] text-muted-foreground">{merchant.businessType}</span>
 <span className="text-sm text-muted-foreground">•</span>
 <span className="text-[13px] text-muted-foreground">Submitted: {merchant.createdAt ? new Date(merchant.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : "10 Sep 2026"}</span>
 <span className="text-sm text-muted-foreground">•</span>
 ...
 <div className="border-y border-border/60 border-border bg-card/50 p-2 flex overflow-x-auto scrollbar-hide">
 ...
 isActive
 ? " shadow-sm border border-border/60 text-primary bg-card border-border dark:text-white"
 : isPassed
 ...
 {/* Tab Content Section */}
 <div className=" bg-card [&>div]:border-0 [&>div]:rounded-none [&>div]:shadow-none">
