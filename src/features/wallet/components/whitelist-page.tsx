...
export default function WhitelistPage() {
 ...
 <Input placeholder="Search accounts..." className="h-10 w-full rounded-xl pl-9 bg-card border-border bg-card/50 border-border" />
 </div>
 ...
 <TableCell className="font-medium text-foreground dark:text-white">{item.id}</TableCell>
 <TableCell>{item.merchantId}</TableCell>
 <TableCell className="font-medium">{item.accountName}</TableCell>
 <TableCell>
 <div className="flex flex-col">
 <span className="font-mono text-xs">{item.accountNumber}</span>
 <span className="text-xs text-muted-foreground">{item.ifsc}</span>
 </div>
 </TableCell>
 <TableCell className="text-muted-foreground">{new Date(item.requestedAt).toLocaleString()}</TableCell>
